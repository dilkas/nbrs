...
\keywords{Bigraphs \and Bigraphical reactive systems \and Probabilistic
  semantics \and Discrete-time Markov chain \and Markov decision process \and
  Spatial models.}
\end{abstract}

\section{Introduction}

TODO: DTMC generation, MDP generation: show, define
...
% TODO: rework, transition from brs to bigraph
...
space. In this paper we use a generalised version of bigraphs, known as
\emph{bigraphs with sharing}
\cite{DBLP:phd/ethos/Sevegnani12,dblp:journals/tcs/sevegnanic15}, that uses
directed acyclic graphs instead of forests to represent node placement, allowing
one node to be contained in the intersection of several other nodes. In this
section we introduce the main concepts and definitions in order to expand on
them in Section~\ref{extensions}.
...
another node (in this case it can even be in an intersection of several nodes).
However, the pictures tend to become much clearer if we instead visualise the
nesting relations between nodes as just a different type of edges. Consider the
bigraph in Fig.~\ref{example_bigraph}. The white ellipses are the \emph{nodes}.
Each node has a type, called \emph{control}, and denoted here by the labels
...
$\mathsf{e}$. Each node has an ordered list of \emph{ports} (not pictured in
the diagram) that can be thought of as sockets for links. Nodes of the same
control have the same number of ports. The number of ports a node of control
...
\begin{definition}[concrete link graph]
  ...
  subsets of $\mathcal{X}$, called respectivelly the \emph{inner} and
...
\section{Extensions to Bigraphs} \label{extensions}
...
TODO: introduce BigraphER \cite{DBLP:conf/cav/SevegnaniC16}
...
Listing~\ref{example_listing}. Furthermore, the BigraphER's visualisation of the
...
rewards can be exported to the various plain text formats of a probabilistic
model checker PRISM \cite{DBLP:conf/cav/KwiatkowskaNP11} for further
quantitative analysis.

TODO: introduce jupyter (with more/new screenshots)
