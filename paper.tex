...
TODO: introduction to bigraphs and everything else
...
\begin{definition}[probabilistic bigraphical reactive system (PBRS)]
  ...
  bigraph $D(r_i)$ such that $g = D(r_i) \circ r_i$. Let $S = \{ (r_i, r'_i, p_i) \mid g' = D(r_i) \circ r'_i \}$, and
  \[ s = \sum_{i=1}^n p_i. \]
...
\begin{definition}[nondeterministic bigraphical reactive system (NBRS)]
  ...
  \[ g \xrightarrowrhd[\rho(a)]{p}_{a} g' \text{ iff } S \ne \emptyset, \]
  where
...
TODO: minimal example, possibly the same one as below.
