...
Akin to \cite{dblp:conf/nfm/giaquintahimn18} and maybe \cite{koenig1998xavier}.
...
Lastly, we have three actions for after the agent acquires the specified number
of objects: two for going home west or south (since these are the two directions
necessary and sufficient to reach home), and one for staying home. The
former type of actions have two reaction rules each, depending on whether the
...
\texttt{goal}. The rule to stay home is quite straightforward and is in
Listing \ref{agent1_home}.
