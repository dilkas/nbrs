...
%TODO: MDP generation: show, define
...
% TODO: introduce BigraphER \cite{DBLP:conf/cav/SevegnaniC16} and its syntax
...
Listing~\ref{example_listing}. BigraphER can construct and visualise the full
transition system, as in Fig.~\ref{example_ts}.
...
ellipse. The labels inside state ellipses list all predicates that are
satisfied by that state. Edges from actions to states are labelled with both the
probability and the name of the reaction rule. Transitions from either a fully
generated transition system or a simulation, labels from predicates, and
